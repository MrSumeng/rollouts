...
// TimeInRange used to validate the expectedTime whether in the timeSlices.
...
func TimeInRange(expectedTime time.Time, allowRunTime *rolloutv1alpha1.AllowRunTime) (time.Time, bool) {
	if allowRunTime == nil {
		return expectedTime, true
	}
	if len(allowRunTime.TimeRanges) == 0 {
		return expectedTime, true
	}
	var (
		err    error
	...
	for i, timeSlice := range allowRunTime.TimeRanges {
